...
// I have kept your actual keys here. 
...
const getApiKey = () => {
  try {
    // This looks for the key you added to Vercel's Environment Variables
...
const App = () => {
  ...
  const analyzeWebsite = async (targetUrl) => {
    if (!apiKey) {
      setError("Missing Gemini API Key. Go to Vercel -> Settings -> Environment Variables and add VITE_GEMINI_API_KEY.");
      return;
    }
    ...
    try {
      const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=${apiKey}`, {
        ...
        body: JSON.stringify({
          ...
          systemInstruction: { parts: [{ text: "Return a JSON bug report with siteScore (number), summary (string), bugs (array), and stats (object with performance, security, accessibility numbers)." }] },
          generationConfig: { responseMimeType: "application/json" }
        })
      });

      if (!response.ok) throw new Error("AI Engine connection failed.");
      ...
      console.error(err);
      setError("Analysis failed. Please try a different URL or check your API key.");
    } finally {
      ...
      <div className="bg-white rounded-[2.5rem] p-10 shadow-sm border border-slate-200">
        ...
        <div className="space-y-6 text-slate-600">
          ...
          <div className="bg-amber-50 border-l-4 border-amber-400 p-4 text-amber-800 text-sm rounded-r-lg">
            <strong>Pro Tip:</strong> Ensure you've added your <code>VITE_GEMINI_API_KEY</code> to Vercel settings to enable the "Analyze" feature.
          </div>
        </div>
                ...
                <div className="bg-white p-10 rounded-[2.5rem] border border-slate-200 shadow-sm">
                  ...
                  <div className="space-y-6">
                    {report.bugs.map((bug, i) => (
                      <div key={i} className="p-8 bg-slate-50 rounded-[2rem] border border-slate-100 hover:border-indigo-200 transition-colors">
